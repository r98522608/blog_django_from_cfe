...
def post_detail(request, pk=None):
    ...
    instance = get_object_or_404(Post, slug=pk)
    if instance.draft:
        if not request.user.is_staff or not request.user.is_superuser:
            raise Http404
    ...
    # the post.model def comments as property
...
def post_update(request, pk=None):
    ...
    if form.is_valid():
        instance = form.save(commit=False)
        instance.save()
        messages.success(request,"<a href='%s'>Item </a> saved" % instance.get_absolute_url(), extra_tags='html_safe')
        return HttpResponseRedirect(instance.get_absolute_url())
    context = {
